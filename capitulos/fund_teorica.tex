...
%Neste capítulo abordaremos alguns dos conceitos necessários para o desenvolvimento e entendimento do projeto, do campo de estudo de processamento de linguagem natural, mais especificamente a análise de sentimentos. Também iremos discorrer sobre a funcionalidade de avaliação de estabelecimentos exposta pelo \textit{Google Maps}.
...
Depois de pré-treinado em um grande corpus de texto, um modelo \textit{BERT} pode ser ajustado para tarefas específicas, como análise de sentimentos, classificação de texto, resposta a perguntas, entre outras, por meio de um processo chamado de ajuste fino (\textit{fine-tuning}). Durante o ajuste fino, o modelo é treinado em um conjunto de dados rotulado específico para a tarefa em questão, adaptando suas representações aprendidas durante o pré-treinamento para a tarefa específica.

Os modelos baseados em \textit{BERT} tornaram-se amplamente populares e são frequentemente usados como base para uma variedade de tarefas de processamento de linguagem natural devido à sua capacidade de capturar contextos complexos e produzir resultados de alta qualidade, \textit{BERTs} treinados em português e com boa performance podemos citar o RoBERTa~\cite{liu2019roberta} e BERTimbau~\cite{souza2020bertimbau}.

\subsubsection{Vicuna}
\label{cap:fund_teorica:sec:modelos:subsec:vicuna}

O Vicuna~\cite{vicuna2023} é um modelo de assistente de chat desenvolvido pela LMSYS~(\textit{Large Model Systems Organization}), ajustado a partir do modelo LLaMA usando ajuste fino de instrução supervisionada. É um modelo de linguagem auto-regressivo baseado na arquitetura \textit{transformers}, projetado para tarefas de geração de texto. Os dados de treinamento para o Vicuna v1.5 consistem aproximadamente de 125.000 conversas coletadas do ShareGPT.com, enquanto o Vicuna v1.1 foi treinado com cerca de 70.000 conversas da mesma fonte.
...
Para aqueles interessados em começar com o Vicuna, o cartão de modelo do \textit{Hugging Face} fornece informações detalhadas sobre detalhes do modelo, fontes, usos e como começar a trabalhar com o modelo. Além disso, há uma nota sobre uma versão mais recente dos pesos disponíveis, sugerindo que os usuários verifiquem atualizações para garantir que estejam usando a versão mais atual do modelo.
...
A análise de sentimentos tem uma ampla gama de aplicações práticas:

\begin{itemize}
    \item \textbf{Monitoramento de Marca}: Empresas utilizam análise de sentimentos para monitorar a percepção de suas marcas nas redes sociais e outras plataformas online \cite{Cambria2017}.
    \item \textbf{Serviço ao Cliente}: Sistemas de atendimento ao cliente automatizados utilizam análise de sentimentos para identificar e priorizar interações negativas que necessitam de atenção imediata \cite{Mishra2019}.
    \item \textbf{Pesquisa de Mercado}: A análise de sentimentos é utilizada para entender as opiniões dos consumidores sobre produtos e serviços, auxiliando no desenvolvimento de estratégias de marketing e melhoria de produtos \cite{Vinodhini2012}.
    \item \textbf{Análise Política}: Governos e organizações utilizam a análise de sentimentos para monitorar a opinião pública e avaliar a reação a políticas e eventos políticos \cite{Bakliwal2013}.
\end{itemize}
